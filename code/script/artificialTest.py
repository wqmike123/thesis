...
cwd = os.getcwd()
W,news, target = trainTool.prepareFakeData(cwd = cwd,isClass = False)
...
sess = tf.Session()
model = vixPredict_cnn(27,len(W),embedweight = W.astype(np.float32),trainable = False,lstm_units=256,learning_rate = 0.01,event_embedding=100,predict_state_dim=_state_dim,pred_dense_dim=128)#,clipvalue=5)
